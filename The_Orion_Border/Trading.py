...
class Traders:
    def __init__(self,price,resource,on_stock,merc_desc):
        ...
        if on_stock is not None:
           ...
           self.merc_on_stock(None,None,None)
        if merc_desc is not None: #description for the selling merc
           self.merc_desc = merc_desc
        else:
           self.merc_desc = '-SOLD OUT-'
    ...
    def resource_asignator(self): #Designed to asign the trading resource depending of the quantity asked (3 or less will be fuel, more than 3 food)
        if self.price <= 3:
         ...
         self.resource = 'food'
    def merc_on_stock(self,ship,merc,trade):
       if merc is False:
          if self.resource == 'food' and ship.food_used(self.price) > 0 or self.resource == 'fuel' and ship.fuel_used(self.price) > 0:
             ...
             if self.resource == 'food':
                ...
                return trade
          elif self.resource == 'food' and ship.food_used(self.price) < 0 or self.resource == 'fuel' and ship.fuel_used(self.price) < 0:
             print('Can\'t affort it')
       elif merc is True:
